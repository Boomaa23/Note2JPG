...
public class SwingConsole extends OutputStream {
    private byte[] oneByte;
    private Appender appender;

    public SwingConsole(JTextArea textArea) {
        this(textArea, 1000);
    }

    public SwingConsole(JTextArea textArea, int maxLine) {
        if (maxLine < 1) {
    ...
    public static class Appender implements Runnable {
        ...
        public synchronized void run() {
            ...
            for (String val : values) {
                ...
                else if (val.startsWith("\r") && !val.isBlank()) {
                    textArea.replaceRange("\n", textArea.getText().lastIndexOf("\n"), textArea.getText().length());
                    lengths.removeLast();
                    lengths.addLast(curLength);
                    curLength = 0;
                }
